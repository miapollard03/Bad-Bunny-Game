...
string Player::dropPoisonedCarrot()
{
    ...
    // Below uses the Arena pointer data member from the Player class to have access the the Arena class which
    // is connected with the History class because we created an object for history which we can reference 
    // public functions from the history class so that we are able to use the record function to know where a
    // carrot was dropped.
